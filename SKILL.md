...
1. `auth_status` — há sessão? Se não: peça ao usuário para rodar `mercadolivre login`
   no terminal (abre o navegador padrão, que precisa ser Chrome, Brave, Edge ou Vivaldi —
   Arc não aceita automação; ele digita a senha lá e a ferramenta nunca a vê) ou
